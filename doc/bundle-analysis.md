...
### 4.（已完成）依赖外置 👉 [doc/tree-shaking-externals.md](./tree-shaking-externals.md)
通过 `EXTERNALS_CDN=true` 把 react/react-dom 外置到 esm.sh CDN,自托管 bundle gzip 从 52 kB 降到 11 kB(降 78%)。同时用 manualChunks 把 sentry 拆到独立 chunk,业务代码变动不影响 vendor 长缓存。
